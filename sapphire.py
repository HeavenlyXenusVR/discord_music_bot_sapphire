...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    ...
    active_key = _track_instance_identity(track_uid, video_url, title)
    await cur.execute(
        "SELECT video_url, title, track_uid FROM sapphire_queue WHERE guild_id = %s AND bot_name = 'sapphire' ORDER BY id ASC LIMIT 25",
        (guild_id,),
    )
    existing_rows = list(await cur.fetchall() or [])
    if any(
        _track_instance_identity(
            _row_track_uid(existing, 2),
...
async def requeue_failed_track(cur, guild_id, channel_id, url, title, requester_id, *, position=0, reason="recovery", track_uid=None):
    ...
    async with get_track_requeue_lock(guild_id):
        ...
        if recent_until and now < recent_until:
            await cur.execute(
                "SELECT video_url, title, track_uid FROM sapphire_queue WHERE guild_id = %s AND bot_name = 'sapphire' ORDER BY id ASC LIMIT 12",
                (guild_id,),
            )
            existing_rows = list(await cur.fetchall() or [])
            already_queued = any(
                _track_instance_identity(
                    _row_track_uid(row, 2),
                    _row_value(row, "video_url", _row_value(row, 0)),
                    _row_value(row, "title", _row_value(row, 1)),
                ) == track_identity
        ...
        await cur.execute(
            "SELECT video_url, title, track_uid FROM sapphire_queue WHERE guild_id = %s AND bot_name = 'sapphire' ORDER BY id ASC LIMIT 8",
            (guild_id,),
        )
        existing_rows = list(await cur.fetchall() or [])
        already_queued = any(
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    try:
                        ...
                        await cur.execute(
                            ...
                            FOR UPDATE
                            """,
                            ('sapphire', DIRECT_ORDER_MAX_ATTEMPTS, DIRECT_ORDER_CLAIM_TIMEOUT_SECONDS, DIRECT_ORDER_FETCH_LIMIT),
                        )
                        ...
                        if ids:
                            placeholders = ','.join(['%s'] * len(ids))
                            await cur.execute(
                                f"""UPDATE sapphire_swarm_direct_orders
                                    SET claimed_at = NOW(), claim_token = %s
                                    WHERE id IN ({placeholders})
                                      AND bot_name = %s
                                      AND COALESCE(attempts, 0) < %s
                                      AND (claimed_at IS NULL OR claimed_at <= NOW() - INTERVAL %s SECOND)""",
                                (DIRECT_ORDER_CLAIM_TOKEN, *ids, 'sapphire', DIRECT_ORDER_MAX_ATTEMPTS, DIRECT_ORDER_CLAIM_TIMEOUT_SECONDS),
                            )
                            await cur.execute(
                                """
