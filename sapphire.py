                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    for g in guilds:
                        ...
                        await cur.execute(
                            ...
                            VALUES (%s, 'sapphire', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0, %s, %s)
                            """,
                            (
                                g.id,
                                voice_connected,
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if reason == "FINISHED":
                            ...
                            if loop_mode == 'queue':
                                ...
                                await insert_queue_front(cur, "sapphire_queue", guild.id, "sapphire", track_uri, track_title, original_requester, track_data.get("track_uid"))
                            else:
            ...
            for finalize_attempt in range(DIRECT_ORDER_DB_RETRY_ATTEMPTS):
                try:
                                try:
                                    await cur.execute("START TRANSACTION")
                                    if executed or attempts + 1 >= DIRECT_ORDER_MAX_ATTEMPTS or not guild:
                                        await cur.execute("DELETE FROM sapphire_swarm_direct_orders WHERE id = %s", (oid,))
                                    else:
                                        await cur.execute("UPDATE sapphire_swarm_direct_orders SET attempts = COALESCE(attempts, 0) + 1, last_error = %s, claimed_at = DATE_SUB(NOW(), INTERVAL %s SECOND), claim_token = NULL WHERE id = %s", (f"unexecuted:{cmd}", DIRECT_ORDER_RETRY_BACKDATE_SECONDS, oid))
                                    await cur.execute("COMMIT")
                                except Exception as tx_error:
                                    try:
                                        await cur.execute("ROLLBACK")
...
class SwarmIntelligence(commands.Cog):
    ...
    async def heartbeat(self):
        try:
            ...
            logger.exception("[sapphire] Heartbeat update failed.")
