...
def prune_runtime_state_cache():
    ...
    for mapping in (process_queue_locks, voice_connect_locks, last_position_persist, recovery_retry_counts, recovery_exhausted_until, idle_voice_since, auto_restore_snooze_until, vote_skip_sessions, metrics_last_errors):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song and allow_recovery_restore:
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT id, video_url, title FROM sapphire_queue WHERE guild_id = %s AND bot_name = 'sapphire' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows:
                    await cur.execute("DELETE FROM sapphire_queue WHERE id = %s AND guild_id = %s AND bot_name = 'sapphire'", (r[0], interaction.guild.id))
                    await cur.execute(
                        "DELETE FROM sapphire_queue_backup WHERE video_url = %s AND title = %s AND guild_id = %s AND bot_name = 'sapphire' LIMIT 1",
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO sapphire_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", (interaction.user.id, name, url, title))
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                res = await cur.fetchone()
                if res and res[0] != 'none': return await interaction.response.send_message(embed=discord.Embed(description="❌ **Conflict:** Disable standard Filters via `/sapphire_main_filter none` first.", color=discord.Color.red()), ephemeral=True)
