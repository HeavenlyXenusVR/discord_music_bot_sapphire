...
![Sapphire bot icon](assets/bot-icon.png)

Sapphire is one node in the 13-bot Discord music fleet. A polished, gem-toned node for servers that want a bit of sparkle. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Sapphire)**
- Internal node name: `sapphire`
- Command namespace: `/sapphire_main_*`
...
All commands are namespaced as `/sapphire_main_<command>`.
...
- Node: `sapphire`
- Database schema: `discord_music_sapphire`
